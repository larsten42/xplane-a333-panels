...
Web-based cockpit panels for X-Plane 12's stock Airbus A330 — currently an
**MCDU (CDU)** and an **EFIS control panel**, both in one page, switched with
a Panel selector in the top bar. It runs in any browser and talks to X-Plane
over the sim's built-in Web API — no X-Plane plugin to install. A small
local server needs to run alongside X-Plane (plain Node.js, no dependencies
— see "Getting started" below), but that's it. Open the page on a tablet on
...
namespaces and need their own profile — see "Adding support for another
aircraft" below. The app itself isn't A330-specific under the hood: every
panel reads its dataref/command mapping from `config/profiles/*.json`, and
it currently ships one verified aircraft's worth of profiles.
...
The gallery above predates the EFIS panel — it's reachable today via the
same page's **Panel** selector, switching between the MCDU shown here and a
QNH/baro readout, ND mode and range selectors, bearing-pointer (ADF/VOR)
toggles, and the CSTR/WPT/VOR.D/NDB/ARPT/FD/LS button row.

## Requirements

- X-Plane 12.1.4 or later — this is when the Web API added command support,
  which keypresses/button presses need (dataref access alone works from
  12.1.1).
...
- Node.js 22.4 or later, installed on the same machine as X-Plane — unless
  you use the single-file executable release, which needs nothing installed
  at all (see below).
...
real X-Plane instance to test against (see "Known limitations").
...
between the page's DOM and X-Plane's datarefs/commands —
`mcdu-adapter.js` decodes the CDU's character-grid datarefs into a screen
model, `efis-adapter.js` maps named buttons/readouts/toggle switches from a
